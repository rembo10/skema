...
createAcquisitionRule :: SQLite.Connection -> Text -> Maybe Text -> SourceType -> Bool -> Maybe Text -> Maybe Int64 -> IO Int64
createAcquisitionRule conn name description sType enabled filters qpId =
  insertReturningId conn
    "INSERT INTO acquisition_rules (name, description, rule_type, enabled, filters, quality_profile_id) \
    \VALUES (?, ?, ?, ?, ?, ?) RETURNING id"
...
updateAcquisitionRule :: SQLite.Connection -> Int64 -> Text -> Maybe Text -> SourceType -> Bool -> Maybe Text -> Maybe Int64 -> IO ()
updateAcquisitionRule conn sid name description sType enabled filters qpId =
  executeQuery conn
    "UPDATE acquisition_rules SET name = ?, description = ?, rule_type = ?, enabled = ?, filters = ?, quality_profile_id = ?, updated_at = CURRENT_TIMESTAMP \
    \WHERE id = ?"
    (name, description, Utils.sourceTypeToText sType, enabled, filters, qpId, sid)
